    ...
    class StatusReporter
      ...
      def update_production_tag(repo_name)
        ...
        if head_commit_id != commit_at_tag
          octokit_client.update_ref(full_repo_name, deploy_ref, head_commit_id)
        else
          raise RefAlreadyDeployed, "HEAD and tag commit SHAs match, ref already deployed"
        end
      end
